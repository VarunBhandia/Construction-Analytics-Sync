	...
	<!-- page content -->
                ...
                <div class="x_content">
					...
					<?php echo '<font style="font-size:16px;" color="green">'.$this->session->flashdata('success_msg').'</font>' ?>
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-10 col-sm-6 col-xs-12">
                           <select class="vendorname form-control" id="vendor" name="vendor" required>
      ...
      $('.vendorname').select2({
        placeholder: '--- Select Vendor ---',
                      ...
                      <div class="form-group">
                        <label class="control-label col-md-2 col-sm-3 col-xs-12" for="last-name">Transporter
                        </label>
                        <div class="col-md-10 col-sm-6 col-xs-12">
                           <select class="trname form-control" id="transporter" name="transporter" required>
								<option value="">---Transporter name----</option>
								<?php
								foreach($transporters as $transporter)
								{ ?>
									<option <?php if($action == 'update'){  echo $transporter->tid == $row[0]->tid ? 'selected' : '' ; }?> value="<?php echo $transporter->tid?>"><?php echo $transporter->tname;?></option>
								<?php }	?>
							</select>
                       <script type="text/javascript">
      $('.trname').select2({
        placeholder: '--- Select Transporter ---',
					  ...
					  <div class="form-group">
                        <label class="control-label col-md-2 col-sm-3 col-xs-12">Return Date
                        </label>
                        <div class="col-md-10 col-sm-6 col-xs-12 form-group">
                          <input class="form-control" id="date" name="date" type="text" value="<?php echo ($action == 'update') ? date("d-m-Y",strtotime($row[0]->rtvreturndate)) :  date("d-m-Y"); ?>" autocomplete="off">
						</div>
                      </div>
                      <div class="form-group">
                        <label class="control-label col-md-2 col-sm-6 col-xs-12" for="name">Vendor Challan No
                        </label>
                        <div class="col-md-10 col-sm-6 col-xs-12">
                          <input type="number" min="1" id="vchallan" name="vchallan" class="form-control col-md-7 col-xs-12" placeholder="Vendor Challan No" value="<?php echo ($action == 'update') ? $row[0]->vchallan : '';?>">
                        </div>
                      </div>
                      <div class="form-group">
                        <label class="control-label col-md-2 col-sm-6 col-xs-12" for="name">Site Challan No
                        </label>
                        <div class="col-md-10 col-sm-6 col-xs-12">
                          <input type="number" min="1" id="schallan" name="schallan" class="form-control col-md-7 col-xs-12" placeholder="Site Challan No" value="<?php echo ($action == 'update') ? $row[0]->schallan : '';?>">
                        </div>
                      </div>
	<div class="table-responsive">		  
		<table class="table table-striped jambo_table" style="width:100%;">
				<thead>
					...
					<th class="column-title">Return Quantity</th>
      ...
      $('.materialname').select2({
        placeholder: '--- Select Material ---',
				...
				<?php } else { ?>
				<input type="hidden" name="rtvid" value="<?php echo $row[0]->rtvid; ?>"/>
				<?php 
				
					$material = explode(",",$row[0]->mid);
					$qty = explode(",",$row[0]->rtvqty);
                    $m_unit = explode(",",$row[0]->muid);
                    $truck = explode(",",$row[0]->rtvtruck);         
					$remark = explode(",",$row[0]->rtvremark);
